...
public:
  ...
  TextOutStream<BinaryStream> & print(uint8_t number) {
    bool nonzero = false;
    uint8_t digit;
    for (digit = 0; number >= 100; number -= 100) digit++;
    if (nonzero || digit != 0) { print((char)('0' + digit)); nonzero = true; }
    for (digit = 0; number >= 10; number -= 10) digit++;
    if (nonzero || digit != 0) { print((char)('0' + digit)); nonzero = true; }
    for (digit = 0; number >= 1; number -= 1) digit++;
    print((char)('0' + digit));
    return *this;
  }
  ...
  TextOutStream<BinaryStream> & print(uint16_t number) {
    bool nonzero = false;
    uint8_t digit;
    for (digit = 0; number >= 10000; number -= 10000) digit++;
    if (nonzero || digit != 0) { print((char)('0' + digit)); nonzero = true; }
    for (digit = 0; number >= 1000; number -= 1000) digit++;
    if (nonzero || digit != 0) { print((char)('0' + digit)); nonzero = true; }
    for (digit = 0; number >= 100; number -= 100) digit++;
    if (nonzero || digit != 0) { print((char)('0' + digit)); nonzero = true; }
    for (digit = 0; number >= 10; number -= 10) digit++;
